...
@section('container')
    <div class="mt-5 mb-5">
        <div class="row">
            <div class="col-md-10"><h3>Edit invoice</h3></div>
        </div>
        @include('alert.validate')
        <form id="buyForm" method="post" action="{{route('invoiceUpdate', $invoice->id)}}">
            @csrf
                ...
                <tbody id="chosenItemsList">
                    @foreach ($listFruitsByInvoice as $fruit)
                        <tr>
                            ...
                            <td>{{$fruit->price}}</td>
                            <td><a href="javascript:void(0)" class="btn-remove" data-fruitId="{{$fruit->id}}">Remove</a></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
